<template>
  <view class="home-container">
      ...
      <view class="content-section">
        ...
        <view class="service-list-soul">
          ...
          <view v-else-if="services.length > 0">
            <view v-for="item in services" :key="item.service_id" class="service-card-soul" @click="goToSubmit(item)">
    ...
    /> 
	<hm-tabbar currentTab="home"></hm-tabbar>
  </view>
</template>
...
const goToSubmit = (item) => {
  uni.navigateTo({
    url: `/subPackages/order/submit?service_id=${item.service_id}&price_template_id=${item.price_template_id || ''}&companion_id=${user.value.application_id || 15}&level_order=${user.value.level_order || ''}&nickname=${user.value.nickname || ''}`
  })
}
