...
import { updateCompanionOnlineStatus, getCompanionSchedule, updateCompanionLocation } from '@/api/user.js'
import { 
...
const updateLocationToServer = async (latitude, longitude, address) => {
  ...
  try {
    ...
    const response = await updateCompanionLocation(requestData)
...
const navigateToOrderDetail = (orderId) => {
  uni.navigateTo({
    url: `/subPackages/fpart/order/detail?orderId=${orderId}&companion_id=${props.applicationInfo.id}`
  })
}
...
const requestSubscribeMessage = async () => {
  const tmplId = 'zovIGlvjM10gOUEZjOB-lOQwISsM0PLqfMtIJPbWf4Y'
...
const goToDataSelect = () => {
  ...
  uni.navigateTo({
    url: `/subPackages/fpart/components/DataSelect?companion_id=${props.applicationInfo.id}`
  })
}
